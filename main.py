...
bot = TeleBot(token="7273092289:AAHzW0I3SB2dfZEO4OvRtWCYB2-TVzv0zgk",num_threads=45,threaded=True,skip_pending=True,parse_mode='html', disable_web_page_preview=True) # توكن بوت الرشق الاساسي هنا
if not db.get('accounts'):
    db.set('accounts', [])
    pass

admin = 6556354444 #الادمن
if not db.get("admins"):
    db.set('admins', [admin, 6556354444])
...
def start_message(message):
    ...
    if user_id in db.get("admins") or user_id == sudo:
        ...
        bot.reply_to(message, 'اهلا بك عزيزي الادمن في الوحه الخاصه بيك', reply_markup=keys_)
    ...
    if not db.get(f'user_{user_id}'):
        ...
        btn2 = btn('قسم الرشق 🥷🏻', callback_data='ps')
        btn3 = btn('اعدادات حسابك ⚙️', callback_data='account')
        btn4 = btn('تجميع نقاط ⛩️️', callback_data='collect')
        btn5 = btn('تحويل نقاط 🧩', callback_data='send')
        btn6 = btn('تحديثات البوت 🕸🕷', url='https://t.me/SM_Service1')
...
def c_rs(call):
    ...
    if data == 'priv_serv':
        ...
        if prem:
            ...
            btn3 = btn('تسليم ارقام',url='https://t.me/MS_GIVEBot')
            ...
            keys = mk(row_width=2)
            keys.add(btn('ما هي الخدمات الخاصة ؟', url='https://t.me/SM_Service1/3'))
...
- ارسل /start داخل الروبوت ثم قم بتحويل رسالة من الروبوت الي هنا لتحصل علي {join_tmoo_bot} نقطة ❇️'''
    ...
    if data == 'change_point_mill':
        ...
        btn4 = btn('الذهاب إلى بوت تمويل المليار',url='https://t.me/EEObot?start=6856378437')
...
• لشراء النقاط تواصل مع المطور : @JB_U7'''
...
def tmmo(msg):
    ...
    if joo['coins'] < int(all):
        ...
        return
    x = bot.reply_to(msg, """⚠️ ارفع البوت @MS_GIVEBot ادمن في قناتك
